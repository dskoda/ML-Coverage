...
def get_radial(name, **kwargs):
    if name == "bessel":
        return BesselRBF(**kwargs)

    elif name == "gaussian":
        return GaussianRBFCentered(**kwargs)

    else:
        raise ValueError(
            f"Radial basis function with \
...
class BesselRBF(nn.Module):
    ...
    .. [#dimenet] Klicpera, Groß, Günnemann:
       ...
       ICLR 2020
    Module from SchNetPack
    """

    def __init__(self, n_rbf: int, cutoff: float, **kwargs):
        """
        Args:
